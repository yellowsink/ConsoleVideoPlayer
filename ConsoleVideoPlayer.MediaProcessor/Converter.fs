...
let buildAnsiEscape (top: SKColor) (btm: SKColor) prevTop prevBtm (target: StringBuilder) =
    ...
    if tChanged || bChanged then
        ...
        if tChanged then
            target.Append $"38;2;%i{top.Red};%i{top.Green};%i{top.Blue}" |> ignore
        ...
        if bChanged then
            target.Append $"48;2;%i{btm.Red};%i{btm.Green};%i{btm.Blue}" |> ignore
...
let processImage (path: string) =
    let lookupData = PixelLookup.makeLookupData (SKBitmap.Decode path)
    ...
    |> List.iter (fun y ->
        ...
        |> List.iter (fun x ->
            ...
            buildAnsiEscape top btm prevTop prevBtm working |> ignore
...
let convertAllImages imageDir =
    printfn "Creating ASCII art           "
    let files = DirectoryInfo(imageDir).EnumerateFiles()
                |> Seq.sortBy (fun f -> int f.Name.[6..(f.Name.Length - 4)])
                |> Seq.toList
    ...
    task {
        ...
        sw.Stop()
        printfn $"Done in {sw.Elapsed.Minutes}m {sw.Elapsed.Seconds}s {sw.Elapsed.Milliseconds}ms"
